  ...
  const handleTakePhoto = async () => {
    ...
    if (status !== "granted") {
      Alert.alert(
        ...
        "Please allow camera and photo library access in settings"
      );
      return;
    }
    ...
    <View className="flex-1 p-5 bg-white">
      ...
      <View className="mt-6 p-4 bg-gray-50 rounded-lg">
        ...
        <Text className="text-gray-500 text-sm">
          1. Place used linens in washing bag
        </Text>
        <Text className="text-gray-500 text-sm">
          2. Take photo of bag location
        </Text>
        <Text className="text-gray-500 text-sm">
          3. Click "Mark as Recycled" to complete
        </Text>
      </View>
    </View>
  );
