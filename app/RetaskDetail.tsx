...
export default function ReDetail() {
  ...
  const { addItemToList } = route.params || {}; // 从父页面接收 addItemToList 方法
  ...
  const handleSave = () => {
    ...
    if (addItemToList) {
      addItemToList(newItem); // 调用父页面传递的添加方法
      console.log("Item added to parent list:", newItem);
    } else {
      console.error("addItemToList is not defined!");
    }

    Alert.alert("Task Saved", "Recycle item details saved successfully.");
    navigation.navigate("RetaskDetail"); // 返回父页面
  };
